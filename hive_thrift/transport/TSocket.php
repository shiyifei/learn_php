...
class TSocket extends TTransport {
  ...
  public function readAll($len) {
    ...
    while (TRUE) {
      ...
      if ($buf === FALSE || $buf === '') {
        $md = stream_get_meta_data($this->handle_);                       //这里的stream_get_meta_data方法很关键
        echo 'arrive here, 111，in '.__METHOD__.",md:".json_encode($md)."\n";  //报出在这里异常的地方
        if ($md['timed_out']) {
        ...
        // echo 'arrive here, 222，in '.__METHOD__.",md:".json_encode($md)."\n";  //正确的流程跟踪
        if ($md['timed_out']) {
          throw new TException('TSocket: timed out reading '.$len.' bytes from '.
                               $this->host_.':'.$this->port_);
        } else {
  ...
  public function read($len) {
    ...
    if ($data === FALSE || $data === '') {
      ...
      echo 'arrive here, in '.__METHOD__.",md:".json_encode($md)."\n";
