...
class TSaslClientTransport extends TTransport {
    ...
    public function open() {
        ...
        echo 'in '.__METHOD__.'arrive here,111<br/>';

        $mechanism = 'PLAIN';
        $header = pack('CN', self::START, strlen($mechanism));
        $this->transport_->write($header . $mechanism);
        echo 'in '.__METHOD__.'arrive here,222<br/>';


        $username = 'shiyifei';
        $password = '6f6fe281ef0d8a2403676b3d4cdbab46'; //随意
        $body = chr(0) . $username . chr(0) . $password;

        echo 'in '.__METHOD__.'arrive here,333<br/>';

        $header = pack('CN', self::COMPLETE, strlen($body));
        $this->transport_->write($header . $body);

        echo 'in '.__METHOD__.'arrive here,444<br/>';

        $data = $this->transport_->readAll(5);
        $array = unpack('Cstatus/Ilength', $data);

        echo 'in '.__METHOD__.'arrive here,555<br/>';

        if ($array['status'] != self::COMPLETE) {
             echo 'in '.__METHOD__.'arrive here,666<br/>';
            throw new TTransportException("not complete");
